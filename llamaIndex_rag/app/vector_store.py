...
class VectorStoreHandler:
    ...
    Methods:
        ...
        process_documents(document_paths): Loads, chunks, embeds, and stores documents in the
            vector store.
    ...
    def __init__(
        ...
        if ollama_host is not None:
            model_kwargs["base_url"] = ollama_host
        ...
        logger.info("Embeddings model %s has been initialized.", model_name)
        splitter_params = splitter_params or dict()
    ...
    def process_documents(self, document_paths: list[str] | str) -> None:
        ...
        Args:
            document_paths (list[str] | str): The path or list of paths to the documents to
                be processed.
        """
    ...
    def get_context(self, query: str) -> list[str]:
        ...
        context: list[str] = (
            [node.text for node in query_results.nodes if isinstance(node, TextNode)]
            if query_results.nodes
            else []
        )
